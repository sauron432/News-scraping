...
OUTPUT_DIR    = "onlinekhabar_headlines"          # folder where per-category CSVs are saved
...
def get_categories_from_nav() -> list[tuple[str, str]]:
    """
    Scrape ONLY top-level categories from OnlineKhabar's main navigation bar.
    Avoids submenus and nested links.
    """
    ...
    nav = soup.find("nav")
    if not nav:
        log.error("Top navigation bar not found.")
        return []

    # Find first UL anywhere inside nav
    main_ul = nav.find("ul")
    if not main_ul:
        log.error("Navigation list not found.")
        return []

    # Only direct children <li> (top-level categories only)
    ...
    log.info(f"Discovered {len(categories)} top-level categories.")
    return categories
...
def main():
    ...
    categories = get_categories_from_nav()
